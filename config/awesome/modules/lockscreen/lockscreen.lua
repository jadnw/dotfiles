...
local lock_animation_icon = wibox.widget({
  ...
  halign = "center",
  valign = "center",
  font = beautiful.icon_font .. " Rounded 64",
...
local lock_screen_box = wibox({
  ...
  bgimage = gears.color.recolor_image(beautiful.lockscreen, beautiful.palette.bg0 .. "00"),
})
awful.placement.maximize(lock_screen_box)

lock_screen_box.bg = beautiful.palette.bg0
lock_screen_box.fg = beautiful.palette.fg1

--- Add lockscreen to each screen
awful.screen.connect_for_each_screen(function(s)
  if s == screen.primary then -- luacheck: no global
    s.mylockscreen = lock_screen_box
  else
    s.mylockscreen =
      lib.ui.screen_mask(s, beautiful.lock_screen_bg or beautiful.exit_screen_bg or beautiful.xbackground)
  end
end)

local function set_visibility(v)
  for s in screen do -- luacheck: no global
...
local split_str = function(s, delimiter)
  local result = {}
...
for i, m in pairs(time_char) do
  local text = lib.ui.colorize_text(m, beautiful.palette.fg1 .. "22")

  var_count = var_count + 1
  local create_dummy_text = true

  for j, _ in pairs(pos_map) do
...
local function activate_word(w)
  for _, m in pairs(char_map[w]) do
    local text = m.text
    m.markup = lib.ui.colorize_text(text, beautiful.palette.accent)
  end
end
...
gears.timer({
  ...
  autostart = true,
  callback = function()
    local extracted_time = os.date("%I" .. ":%M")
    local h, m = extracted_time:match("(%d+):(%d+)")
...
local arc = function()
  return function(cr, width, height)
    gears.shape.arc(cr, width, height, 8, 0, math.pi / 2, true, true)
  end
end
...
local function reset()
  ...
  lock_animation_icon:set_text(lock_screen_symbol)
...
local function fail()
  characters_entered = 0
  lock_animation_icon:set_text(lock_screen_fail_symbol)
  lock_animation_widget_rotate.direction = "north"
  lock_animation_arc.bg = "#00000000"
end
...
local function key_animation(char_inserted)
  ...
  if char_inserted then
    ...
    if characters_entered == 0 then
      ...
      color = beautiful.palette.fg1 .. "55"
    end
  end
