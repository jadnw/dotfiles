...
return function()
  ...
  local clear = factory.create_button({
    ...
    border_width = dpi(1),
    border_color = beautiful.palette.yellow,
    fg = beautiful.palette.yellow,
  ...
  local notification_center = factory.create_popup({
    child = {
      {
        {
          {
  ...
  awesome.connect_signal("notification_center::toggle", notification_center.toggle_visible) -- luacheck: no global
  -- Close Notifications Center after clearing the history
  clear:connect_signal("button::release", function()
    notification_center.toggle_visible()
  end)
