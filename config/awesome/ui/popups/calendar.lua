...
return function(s)
	...
	local styles = {
		...
		focus = {
			...
			bg_color = beautiful.palette.accent,
			markup = function(t)
				return "<b>" .. t .. "</b>"
			end,
			shape = default_shape,
		...
		header = {
			fg_color = beautiful.palette.accent,
		},
		weekday = {
	...
	local prev_month = factory.create_button({
		...
		fg_hover = beautiful.palette.accent,
		on_click = function()
	...
	local calendar_popup = factory.create_popup({
		...
		placement = function(w)
			awful.placement.bottom_right(w, {
				margins = { right = beautiful.useless_gap + dpi(2), bottom = beautiful.wibar_height + beautiful.useless_gap + dpi(2) },
			})
		end,
		width = dpi(320),
		height = dpi(360),
